...
function initQuizPage() {
  ...
  if (!currentQuiz) {
    title.innerText = "존재하지 않는 퀴즈입니다.";
    document.getElementById("start-section").style.display = "none";
    result.innerText = "문제 목록에서 다시 선택해주세요.";
    return;
  }

  document.title = currentQuiz.quizTitle;
  title.innerText = currentQuiz.quizTitle;
  loadQuiz(`./data/${currentQuiz.file}`);
}
...
function loadQuiz(filePath) {
  fetch(filePath)
    .then((response) => {
      if (!response.ok) {
        throw new Error(`${currentQuiz.file} 파일을 불러오지 못했습니다.`);
      }
